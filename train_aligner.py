...
model.set_constants(reduction_factor=1)
output = model.predict(test_phonemes[0], encode=False)
audio = Audio(config)
f = audio.display_mel(output['mel'].numpy().transpose())
f.savefig('test.png')
wav = tf.transpose(output['mel'])
wav = audio.reconstruct_waveform(wav)
librosa.output.write_wav('test.wav', wav, sr=22050)
print(wav.shape)
exit(0)
