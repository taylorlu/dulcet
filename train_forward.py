...
def jitter(spk_emb_dim):
    radius = np.abs(np.random.normal(scale=0.25))
    rVector = np.random.uniform(size=[1000, spk_emb_dim])
    rVector /= np.linalg.norm(rVector, axis=1, keepdims=True)
    rVector *= radius
    return rVector
